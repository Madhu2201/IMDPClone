    ...
    const fetchMovie = async () => {
      try {
        const res = await axios.get(
          `https://backend-1h5d.onrender.com/api/getMovieById/${id}`
        );
        setMovie(res.data);
      } catch (err) {
      ...
      <div className="update-delete-border">
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          <input
            ...
            name="coActor"
            className="update-delete-text"
            value={movie.coActor?.join(", ") || ""}
            onChange={handleChange}
            placeholder="Co-Actors (comma-separated)"
          />
