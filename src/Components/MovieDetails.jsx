    ...
    useEffect(() => {
        const fetchMovieData = async () => {
            try {
                ...
                const response = await fetch(`https://backend-1h5d.onrender.com/api/getMovieById/${id}?populate=producer,actors`);
                const movieData = await response.json();
                setMovie(movieData);

                // If population didn't work, fetch details separately
                if (!movieData.producer?.name || !movieData.actors?.[0]?.name) {
                    const producerPromise = fetch(`https://backend-1h5d.onrender.com/api/getProducerById/${movieData.producer}`)
                        .then(res => res.json());
                    
                    const actorsPromises = movieData.actors.map(actorId => 
                        fetch(`https://backend-1h5d.onrender.com/api/getActorById/${actorId}`)
                            .then(res => res.json())
                    );
